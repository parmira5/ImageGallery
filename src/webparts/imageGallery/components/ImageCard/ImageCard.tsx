...
const ImageDetails = ({ description, title, id }: IImageDetails) => {
    ...
    React.useState(() => {
        imageService.getComments(id).then(res => setCommentCount(res.length)).catch(console.error)
    })

    return <div className={styles.imageDetails}>
        <div className={styles.commentDetails}>
            <Icon styles={chatIconStyles} iconName="CommentSolid" />
            <Text styles={textStyles} variant="xLarge">{commentCount}</Text>
        </div>
        <Text variant="mediumPlus" styles={textStyles}>{description}</Text>
    </div>
}
...
const ImageCard = ({ src, description, title, id, onClick }: IImageCard) => {
    ...
    return <div className={styles.imageCard} onClick={handeImageCardClick}>
