...
export class ImageService {
  ...
  public async getImages(): Promise<Post[]> {
    ...
    const commentRepos = await this.batchGetCommentCount(posts.map(post => post.id)).catch(console.error)
    posts.forEach(post => {
      post.comments = new CommentsRepository(find(commentRepos!, (commentRepo: ICommentsServerObj) => commentRepo.value[0]?.itemId == post.id))
    })
    console.log(posts)
    return posts
  }
  ...
  public async getComments(imageId: number, nextPage?: string): Promise<CommentsRepository> {
    const uri = nextPage || `https://r3v365.sharepoint.com/sites/NEWSSITE2/_api/web/lists/getByTitle('Image Gallery')/items('${imageId}')/comments?$inlineCount=AllPages&$top=1`;
  ...
  private async batchGetCommentCount(postIds: number[]): Promise<ICommentsServerObj[]> {
    const batchId = Guid.newGuid().toString();
    const batchIdentifierTemplate =
      `--batch_${batchId}\nContent-type: application/http\nContent-Transfer-Encoding: binary`

    const requestTemplate =
      `GET https://r3v365.sharepoint.com/sites/NEWSSITE2/_api/web/lists/getByTitle('<<LIST>>')/items(<<ID>>)/comments?$inlineCount=AllPages&%24top=1 HTTP/1.1\naccept: application/json`

    let requestBody = ""

    postIds.forEach(postId => {
      requestBody += `${batchIdentifierTemplate}\n\n${requestTemplate.replace("<<LIST>>", "Image%20Gallery").replace("<<ID>>", postId.toString())}\n\n\n\n`
    })
    requestBody = `${requestBody}\n\n--batch_${batchId}--`
    const res = await this._spHttpClient.post("https://r3v365.sharepoint.com/sites/NEWSSITE2/_api/$batch", SPHttpClient.configurations.v1, { headers: { "Content-Type": `multipart/mixed; boundary=batch_${batchId}` }, body: requestBody });
