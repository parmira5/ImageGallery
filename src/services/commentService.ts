...
export class CommentService {
  ...
  private _odataAddOns = "?$inlineCount=AllPages&%24top=20";
  ...
  public init(serviceScope: ServiceScope, spHttpClient: SPHttpClient): void {
    ...
    serviceScope.whenFinished(() => {
      ...
      this._baseQuery = `${configService.sourceSitePath}/_api/web/lists/getById('${configService.galleryId}')`;
    });
  }

  public async getComments(post: number): Promise<CommentObj[]> {
    const base = this._baseQuery;
    const odata = this._odataAddOns;
    const resp = await this._spHttpClient.get(
      `${base}/items/getById('${post}')/comments${odata}`,
  ...
  public async postComment(imageId: number, text: string): Promise<IComment & ICommentInfo> {
    const sp = spfi(configService.sourceSitePath).using(SPFx({ pageContext: this._pageContext }));
    return sp.web.lists.getById(configService.galleryId).items.getById(imageId).comments.add(text);
  }
  ...
  public async batchGetCommentCount(posts: Post[]): Promise<ICommentsServerObj[]> {
    ...
    const requestTemplate = `GET ${this._baseQuery}/items(<<ID>>)/comments?$inlineCount=AllPages&%24top=1 HTTP/1.1\naccept: application/json`;
    let requestBody = "";
    posts.forEach((post) => {
      if (post.id)
        requestBody += `${batchIdentifierTemplate}\n\n${requestTemplate.replace("<<ID>>", post.id.toString())}\n\n\n\n`;
    });
